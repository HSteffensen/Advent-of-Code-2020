...
func InputToPasswordVal(input string) []PasswordValidate {
	lines := strings.Split(input, "\n")
	pwVal := make([]PasswordValidate, 0, 20)
	pattern := "(?P<min>\\d+)-(?P<max>\\d+) (?P<char>\\w): (?P<pw>.+)"
	regexLine := regexp.MustCompile(pattern)
	for _, line := range lines {
		matches := regexLine.FindStringSubmatch(line)
		lineMap := make(map[string]string)
		for i, name := range regexLine.SubexpNames() {
			lineMap[name] = matches[i]
		}
		min, err1 := strconv.Atoi(lineMap["min"])
		max, err2 := strconv.Atoi(lineMap["max"])
		if err1 != nil || err2 != nil {
			fmt.Println("Error in input string to integer list conversion:", input)
		}
		pwVal = append(pwVal, PasswordValidate{
			arg1:     min,
			arg2:     max,
			required: lineMap["char"][0],
			password: lineMap["pw"],
		})
	}
	return pwVal
...
func CheckPasswordPart1(pw PasswordValidate) bool {
	count := strings.Count(pw.password, string(rune(pw.required)))
	return count >= pw.arg1 && count <= pw.arg2
}

func CheckPasswordPart2(pw PasswordValidate) bool {
	char1 := pw.password[pw.arg1-1]
	char2 := pw.password[pw.arg2-1]
	return (char1 == pw.required) != (char2 == pw.required)
}

func CountValidPasswords(pws []PasswordValidate, validation func(PasswordValidate) bool) int {
	count := 0
	for _, pw := range pws {
		if validation(pw) {
			count++
