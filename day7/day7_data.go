...
var Input = `light gold bags contain 2 light lime bags, 1 faded green bag, 3 clear olive bags, 2 dim bronze bags.
...
func ReadInput(input string) LuggageRules {
	lines := strings.Split(input, "\n")
	allBags := make(LuggageRules)
	for _, line := range lines {
		halves := strings.SplitN(line, " bags contain ", 2)
		container := allBags.Bag(halves[0])
		within := halves[1]
		if within == "no other bags." {
			continue
		}
		for _, inRule := range strings.Split(within[:len(within)-1], ", ") {
			ruleWords := strings.Fields(inRule)
			ruleCount, err := strconv.Atoi(ruleWords[0])
			if err != nil {
				fmt.Printf("day7 ReadInput: Bad number of bags: %#v\n", ruleWords[0])
			}
			ruleBagName := strings.Join(ruleWords[1:3], " ")
			ruleBag := allBags.Bag(ruleBagName)
			container.Holds[ruleBagName] = ruleCount
			ruleBag.HeldBy = append(ruleBag.HeldBy, container.Name)
			allBags[ruleBagName] = *ruleBag
		}
	}
	return allBags
}
